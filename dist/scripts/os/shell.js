...
(function (TSOS) {
    var Shell = (function () {
        ...
        Shell.prototype.init = function () {
            ...
            this.commandList[this.commandList.length] = sc;
            // recover
            sc = new TSOS.ShellCommand(this.shellRecoverFile, "recover", "<filename, *> - recovers file with filename specified from trash, or use * to recover all files from trash.");
            this.commandList[this.commandList.length] = sc;
            //write - write   the data    inside  the quotes  to  filename    and display a   message denoting    success or  failure.    
            sc = new TSOS.ShellCommand(this.shellWriteFile, "write", "<filename>, <-append, -overwrite>, 'data' - writes to data to specified file name. Defaults to"
        ...
        Shell.prototype.execute = function (fn, args) {
            var nonPrompt = fn !== this.shellCreateFile && fn !== this.shellFormatDisk && fn !== this.shellTrashFiles
        ...
        Shell.prototype.shellFormatDisk = function (args) {
            ...
            if (firstParam === undefined && _CPU.isExecuting) {
                ...
                _KernelInterruptQueue.enqueue(new TSOS.Interrupt(FILESYSTEM_IRQ, [DiskAction.FullFormat]));
                return;
            }
            if (firstParam === "-force") {
            ...
            if (firstParam === "-quick" && !_CPU.isExecuting) {
                ...
                _KernelInterruptQueue.enqueue(new TSOS.Interrupt(FILESYSTEM_IRQ, [DiskAction.QuickFormat]));
                return;
            }
        ...
        Shell.prototype.shellCreateFile = function (args) {
            ...
            if (!_FileNames.inQueue(firstParam)) {
                ...
                _KernelInterruptQueue.enqueue(new TSOS.Interrupt(FILESYSTEM_IRQ, [DiskAction.Create, firstParam]));
            }
            else if (secondParam === "-force") {
                //create the file
                _StdOut.putText("Creating File: " + firstParam);
                _KernelInterruptQueue.enqueue(new TSOS.Interrupt(FILESYSTEM_IRQ, [DiskAction.CreateForce, firstParam]));
            }
            else {
        ...
        Shell.prototype.shellDeleteFile = function (args) {
            ...
            if (fileName === undefined) {
                ...
                _KernelInterruptQueue.enqueue(new TSOS.Interrupt(FILESYSTEM_IRQ, [DiskAction.DeleteAll]));
                return;
            }
                ...
                if (force) {
                    _KernelInterruptQueue.enqueue(new TSOS.Interrupt(FILESYSTEM_IRQ, [DiskAction.DeleteForce, fileName]));
                }
                else
                    _KernelInterruptQueue.enqueue(new TSOS.Interrupt(FILESYSTEM_IRQ, [DiskAction.Delete, fileName]));
                return;
            }
            else {
        ...
        Shell.prototype.shellRecoverFile = function (args) {
            var fileName = args[0];
            if (fileName === undefined) {
                _StdOut.putText("Please specify a file name.");
                _OsShell.putPromptNextLine();
                ...
                _KernelInterruptQueue.enqueue(new TSOS.Interrupt(FILESYSTEM_IRQ, [DiskAction.RecoverAll]));
                return;
            }
            else if (_Trash.inQueue(fileName)) {
                //delete existing file
                _StdOut.putText("Recovering File: " + fileName);
                _KernelInterruptQueue.enqueue(new TSOS.Interrupt(FILESYSTEM_IRQ, [DiskAction.Recover, fileName]));
                return;
            }
            else {
                _StdOut.putText("Invalid file name. ");
        ...
        Shell.prototype.shellWriteFile = function (args) {
            ...
            if (typeOfWrite === '-append') {
                if (_FileNames.inQueue(fileName)) {
                    ...
                    _KernelInterruptQueue.enqueue(new TSOS.Interrupt(FILESYSTEM_IRQ, [DiskAction.AppendWrite, fileName, data]));
                }
                else {
                ...
                _KernelInterruptQueue.enqueue(new TSOS.Interrupt(FILESYSTEM_IRQ, [DiskAction.Write, fileName, data]));
            }
        };
        Shell.prototype.shellReadFile = function (args) {
            ...
            if (!_FileNames.inQueue(fileName) && sudo !== "sudo") {
                ...
                _KernelInterruptQueue.enqueue(new TSOS.Interrupt(FILESYSTEM_IRQ, [DiskAction.Read, fileName]));
            }
        };
        ...
        Shell.prototype.shellTrashFiles = function (args) {
            if (args[0] === "-empty") {
                if (_Trash.isEmpty()) {
                    ...
                    _KernelInterruptQueue.enqueue(new TSOS.Interrupt(FILESYSTEM_IRQ, [DiskAction.EmptyTrash]));
                    return;
                }
            }
            if (_Trash.isEmpty()) {
